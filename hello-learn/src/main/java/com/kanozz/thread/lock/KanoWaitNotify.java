package com.kanozz.thread.lock;

import com.kanozz.util.thread.ThreadNameFactory;
...
public class KanoWaitNotify {
    ...
    public static void main(String[] args) throws Exception{
        ...
        log.info("no = {}", ClassLayout.parseInstance(mutex).toPrintable());
        synchronized (mutex){
            log.info("basic = {}", ClassLayout.parseInstance(mutex).toPrintable());
        }
        Thread thread = new Thread(() -> {
            try {
                synchronized (mutex) {
                    log.info("light = {}", ClassLayout.parseInstance(mutex).toPrintable());
        ...
        synchronized (mutex){
            log.info("weight = {}", ClassLayout.parseInstance(mutex).toPrintable());
        }
        thread.join();
